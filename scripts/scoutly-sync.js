...
/**
 * SCOUTLY SYNC V13.3
 * Melhorias sobre V13.2:
 * [1] Threshold de probabilidade elevado: 0.64 → 0.68 geral, equilibrado 0.66 → 0.72
 * [2] buildTeamCornerCandidates reativado (escanteios por time)
 * [3] home/away form persistido no match_analysis (coluna data jsonb)
 */
...
const MIN_PROBABILITY_GENERAL = 0.68      // era 0.64
const MIN_PROBABILITY_EQUILIBRADO = 0.72  // era 0.66
const MIN_PROBABILITY_DEFENSIVO = 0.72    // era 0.70
const MIN_PROBABILITY_INTERNATIONAL = 0.70 // era 0.67
...
const TARGET_COMPETITIONS = [
  ...
  // ===== ARGENTINA =====
  { mode:"country", country:"Argentina", type:"league", names:["Liga Profesional Argentina","Primera División"], display:"Liga Argentina", region:"general", priority:84 },
  // ===== EUROPA =====
...
function round(value, decimals = 2) { return Number(Number(value || 0).toFixed(decimals)) }
function clamp(value, min, max) { return Math.max(min, Math.min(max, value)) }
function sum(arr) { return arr.reduce((acc, value) => acc + value, 0) }
...
function uniqBy(arr, getKey) {
  const seen = new Set()
  return arr.filter((item) => { const key = getKey(item); if (seen.has(key)) return false; seen.add(key); return true })
}
...
async function api(path, params = {}) {
  ...
  const response = await fetch(url, { method:"GET", headers:{ "x-apisports-key": APISPORTS_KEY } })
  if (!response.ok) { const text = await response.text(); throw new Error(`API ${response.status} em ${path}: ${text}`) }
...
function hasForbiddenMarker(value = "") {
  ...
  return (
    v.includes("u17")||v.includes("u18")||v.includes("u19")||v.includes("u20")||
    v.includes("u21")||v.includes("u23")||v.includes("under 17")||v.includes("under 18")||
    v.includes("under 19")||v.includes("under 20")||v.includes("under 21")||v.includes("under 23")||
    v.includes("sub 17")||v.includes("sub 18")||v.includes("sub 19")||v.includes("sub 20")||
    v.includes("sub 21")||v.includes("sub 23")||v.includes("women")||v.includes("woman")||
    v.includes("female")||v.includes("feminino")||v.includes("feminina")||v.includes("femenil")||
    v.includes("frauen")||v.includes("vrouwen")||v.includes("reserve")||v.includes("reserves")||v.includes("youth")
  )
}
...
function isExactTargetLeague(target, rawName, country) {
  ...
  if (target.country === "Mexico" && target.display === "Liga MX") return c === "mexico" && raw === "liga mx"
...
function isLikelyClubName(name = "") {
  ...
  const clubMarkers = ["fc","sc","afc","cf","ac","club","united","city","rovers","athletic","atletico","deportivo","sporting","jk","fk","bk","if"]
...
function isClubFriendlyFixture(fixture) {
  ...
  if (isLikelyClubName(homeName) || isLikelyClubName(awayName)) return true
...
function isInternationalCompetition(comp, fixture = null) {
  ...
  return (
    region === "international" || display.includes("amistosos internacionais") ||
    display.includes("nations league") || display.includes("eliminatorias") ||
    display.includes("eurocopa") || display.includes("copa america") ||
    display.includes("copa do mundo") || leagueName.includes("friendlies") || country === "world"
  )
}
...
function normalizeCompetitionName(country, rawName, fallbackDisplay) {
  ...
  if (c === "Brazil" && (norm.includes("sul-sudeste") || norm.includes("sul sudeste"))) return "Copa Sul-Sudeste"
  if (c === "Brazil" && (norm.includes("women") || norm.includes("feminino") || norm.includes("feminina"))) return "Brasileirão Feminino"
  if (c === "Argentina" && (name === "Liga Profesional Argentina" || name === "Primera División")) return "Liga Argentina"
  ...
  const items = leagues
    .map((item) => {
      ...
      if (target.display === "Saudi Pro League") {
        ...
        const isProLeague = rawLower === "pro league" || rawLower.includes("saudi pro league") || rawLower.includes("spl")
        if (!isSaudi || !isProLeague) return null
      }
      if (!isExactBrazilRegionalMatch(target.display, country, rawName)) return null
      if (target.display === "Amistosos Internacionais") { if (!haystack.includes("friend")) return null }
      if (target.display === "Nations League") { if (!haystack.includes("nations")) return null }
      return {
        leagueId: item.league.id,
        season: currentSeason.year,
        country,
...
async function fetchFixturesForCompetition(comp) {
  ...
  try {
    const fixtures = await api("/fixtures", {
      league: comp.leagueId, season: comp.season,
      from: isoDate(start), to: isoDate(end), timezone: TIMEZONE,
    })
    const cleaned = fixtures
      .filter((fixture) => {
        ...
        if (compDisplay.includes("amistosos internacionais")) { if (isClubFriendlyFixture(fixture)) return false }
        return true
      })
...
function extractStatValue(statistics = [], type) {
  ...
  if (typeof value === "string") { const cleaned = value.replace("%", "").trim(); const num = Number(cleaned); return Number.isFinite(num) ? num : 0 }
...
function weightedAverage(rows, key, fallback = 0) {
  ...
  const weightedSum = rows.reduce((acc, row, index) => acc + safeNumber(row[key], 0) * weights[index], 0)
  return weightedSum / totalWeight
}

function splitVenueFixtures(fixtures, teamId, wantHome, limit = FORM_LIMIT_HOME_AWAY) {
  return fixtures
    .filter((fixture) => { const isHome = fixture?.teams?.home?.id === teamId; return wantHome ? isHome : !isHome })
    .slice(0, limit)
}

// [3] Detecta streak de forma (em alta / em queda / irregular)
function detectFormStreak(fixtures, teamId, limit = 5) {
  const recent = fixtures.slice(0, limit)
  if (!recent.length) return "sem dados"
  const results = recent.map((fixture) => {
...
async function collectProfileFromFixtures(teamId, fixturesSubset) {
  if (!fixturesSubset.length) {
    return { matches:0, statsMatches:0, avgGoalsFor:0, avgGoalsAgainst:0, avgShots:0, avgShotsOnTarget:0, avgCorners:0, avgCards:0, avgFouls:0, recentScores:[], formStreak:"sem dados" }
  }
  const rows = []
  for (const fixture of fixturesSubset) {
    ...
    rows.push({
      ...
      cards: extractStatValue(teamStats, "Yellow Cards") + extractStatValue(teamStats, "Red Cards"),
      scoreLabel: buildScoreLabelForTeam(fixture, teamId),
    })
  }
  const statsRows = rows.filter((row) => row.shots > 0 || row.shotsOnTarget > 0 || row.corners > 0 || row.fouls > 0 || row.cards > 0)
  const formStreak = detectFormStreak(fixturesSubset, teamId)
  return {
    matches: rows.length, statsMatches: statsRows.length,
...
function isUsableTeamProfile(sideProfile, generalProfile) {
  return safeNumber(generalProfile.matches, 0) >= MIN_REQUIRED_RECENT_MATCHES &&
         safeNumber(generalProfile.statsMatches, 0) >= MIN_REQUIRED_STATS_MATCHES
}

function buildExpectedMetrics(homeProfile, awayProfile) {
  const expectedHomeGoals = clamp(round(homeProfile.avgGoalsFor * 0.60 + awayProfile.avgGoalsAgainst * 0.40), 0.25, 3.8)
  const expectedAwayGoals = clamp(round(awayProfile.avgGoalsFor * 0.56 + homeProfile.avgGoalsAgainst * 0.44), 0.20, 3.4)
  ...
  const expectedCorners = clamp(round(
    homeProfile.avgCorners * 0.50 + awayProfile.avgCorners * 0.46 +
    expectedShots * 0.050 + expectedSOT * 0.045 + pressureFactor
  ), 4.5, 13.2)
  const expectedCards = clamp(round(
  ...
  return {
    ...
    expectedHomeShots, expectedAwayShots, expectedHomeSOT, expectedAwaySOT,
    expectedShots, expectedSOT, expectedCorners, expectedCards, expectedFouls,
  }
}
...
function buildProbabilities(metrics) {
  ...
  return {
    home: round(clamp(homeWin, 0.05, 0.88)), draw: round(clamp(draw, 0.06, 0.42)),
    away: round(clamp(awayWin, 0.05, 0.88)), over15: round(clamp(over15, 0.10, 0.97)),
    over25: round(clamp(over25, 0.08, 0.94)), btts: round(clamp(btts, 0.08, 0.92)),
    under35: round(clamp(under35, 0.12, 0.97)), corners: round(cornersProb),
    shots: round(shotsProb), sot: round(sotProb), cards: round(cardsProb),
  }
}

function buildMarkets(metrics, probs) {
  return {
    over15: probs.over15, over25: probs.over25, btts: probs.btts, under35: probs.under35,
    corners: round(metrics.expectedCorners), cards: round(metrics.expectedCards),
    shots: round(metrics.expectedShots), shots_on_target: round(metrics.expectedSOT),
...
function buildMetrics(metrics) {
  return {
    goals: round(metrics.expectedGoals), corners: round(metrics.expectedCorners),
    shots: round(metrics.expectedShots), shots_on_target: round(metrics.expectedSOT),
    cards: round(metrics.expectedCards), fouls: round(metrics.expectedFouls),
  }
}
...
function buildCornerCandidates(metrics) {
  ...
  function add(market, probability) {
    if (!market) return
    candidates.push({ market, probability: round(probability), score: lineScore(probability, "escanteios", market), family: "escanteios" })
  }
...
function buildShotsCandidates(metrics, probs) {
  ...
  function add(market, probability) {
    if (!market) return
    candidates.push({ market, probability: round(probability), score: lineScore(probability, "shots", market), family: "shots" })
  }
  const shots = safeNumber(metrics.expectedShots, 0)
...
function buildSOTCandidates(metrics, probs) {
  ...
  function add(market, probability) {
    if (!market) return
    candidates.push({ market, probability: round(probability), score: lineScore(probability, "sot", market), family: "sot" })
  }
...
// [2] buildTeamCornerCandidates REATIVADO
...
function buildCardsCandidates(metrics, probs) {
  ...
  function add(market, probability) {
    if (!market) return
    candidates.push({ market, probability: round(probability), score: lineScore(probability, "cards", market), family: "cards" })
  }
...
function buildCandidateMarkets(payload) {
  ...
  function add(market, probability, family) {
    if (!market) return
    candidates.push({ market, probability: round(probability), score: lineScore(probability, family, market), family })
  }
  ...
  // [2] buildTeamCornerCandidates REATIVADO
  buildTeamCornerCandidates({ homeTeam, awayTeam, metrics }).forEach((item) => candidates.push(item))
  ...
  if (homeOrDraw >= 0.74) {
    const allowed = !isNationalTeamsGame || mismatch <= STRONG_MISMATCH_DOUBLE_CHANCE_BLOCK || homeProb >= awayProb
    if (allowed) add(`Dupla chance ${homeTeam} ou empate`, isNationalTeamsGame ? Math.min(homeOrDraw, NATIONAL_TEAM_DOUBLE_CHANCE_CAP) : homeOrDraw, "dupla_chance")
  }
  if (awayOrDraw >= 0.74) {
    const allowed = !isNationalTeamsGame || mismatch <= STRONG_MISMATCH_DOUBLE_CHANCE_BLOCK || awayProb >= homeProb
    if (allowed) add(`Dupla chance ${awayTeam} ou empate`, isNationalTeamsGame ? Math.min(awayOrDraw, NATIONAL_TEAM_DOUBLE_CHANCE_CAP) : awayOrDraw, "dupla_chance")
  }
...
function normalizeLeagueByTeams(comp, fixture) {
  ...
  if (normalizeText(teams).includes("fluminense w") || normalizeText(teams).includes("corinthians w") || normalizeText(teams).includes("palmeiras w")) {
    if (normCountry === "brazil") leagueDisplay = "Brasileirão Feminino"
  }
...
function buildInsight(mainPick, metrics, profile) {
  ...
  if (mainPick === "Mais de 1.5 gols") return `A leitura Scoutly projeta um confronto com boa chance de pelo menos 2 gols. A média esperada está em ${goals} gols, com cenário ofensivo sustentável para essa linha.`
  if (mainPick === "Menos de 2.5 gols") return `O modelo identifica um cenário mais travado, com baixa explosão ofensiva e bom suporte estatístico para até 2 gols no jogo.`
  if (mainPick === "Menos de 3.5 gols") return `O modelo identifica um cenário mais controlado, com boa sustentação estatística para até 3 gols no jogo.`
  if (normalizeText(mainPick).includes("escanteios")) {
    if (normalizeText(mainPick).includes("mais de")) return `A leitura Scoutly projeta cerca de ${corners} escanteios, com contexto de jogo favorável para uma linha de cantos mais alta sem forçar uma projeção exagerada.`
    if (normalizeText(mainPick).includes("menos de")) return `A leitura Scoutly projeta cerca de ${corners} escanteios, indicando um cenário mais controlado para o mercado de cantos, sem necessidade de esticar demais a linha.`
  ...
  if (normalizeText(mainPick).includes("finalizações") && !normalizeText(mainPick).includes("no gol")) return `A leitura Scoutly projeta cerca de ${shots} finalizações totais, sugerindo um cenário de volume ofensivo consistente para transformar produção em oportunidade real de mercado.`
  if (normalizeText(mainPick).includes("no gol")) return `A leitura Scoutly projeta cerca de ${sot} finalizações no gol, indicando um cenário de boa produção ofensiva com precisão suficiente para sustentar esse mercado.`
  if (normalizeText(mainPick).includes("cart")) return `A leitura Scoutly projeta cerca de ${cards} cartões, sugerindo um confronto com nível de contato e tensão suficiente para transformar disciplina em oportunidade de mercado.`
  if (normalizeText(mainPick).includes("dupla chance")) return `A leitura Scoutly aponta vantagem competitiva para um dos lados, mas com proteção ao empate. O equilíbrio da partida ainda pede segurança, e por isso a dupla chance aparece como leitura mais sólida.`
  if (mainPick === "Ambas não marcam") return `A leitura Scoutly vê um confronto com menor troca ofensiva entre os lados, sustentando o cenário de uma equipe passar em branco.`
  if (mainPick === "Ambas marcam") return `A leitura Scoutly identifica espaço para gols dos dois lados, combinando projeção ofensiva e comportamento recente das equipes.`
...
function isInternationalNationalTeamsFixture(fixture, comp) {
  ...
  if (normalizeText(comp?.display || "").includes("amistosos internacionais") || normalizeText(fixture?.league?.name || "").includes("friend")) {
    if (isClubFriendlyFixture(fixture)) return false
  }
  return true
}
...
async function upsertMatch(match) {
  const payload = {
    id: match.id, kickoff: match.kickoff, league: match.league,
    country: match.country || null, region: match.region || null,
    priority: match.priority || null, home_team: match.home_team || null,
    away_team: match.away_team || null, home_logo: match.home_logo || null,
    away_logo: match.away_logo || null, probabilities: match.probabilities || null,
    markets: match.markets || null, metrics: match.metrics || null,
    pick: match.pick || null, probability: match.probability || null,
    insight: match.insight || null, updated_at: new Date().toISOString(),
  }
...
async function upsertMatchStats(row) {
  const payload = {
    ...
    home_shots: row.home_shots, home_shots_on_target: row.home_shots_on_target,
    home_corners: row.home_corners, home_yellow_cards: row.home_yellow_cards,
    away_shots: row.away_shots, away_shots_on_target: row.away_shots_on_target,
    away_corners: row.away_corners, away_yellow_cards: row.away_yellow_cards,
...
// [3] upsertMatchAnalysis agora persiste home/away form na coluna data (jsonb)
async function upsertMatchAnalysis(row) {
  const payload = {
    match_id: row.match_id,
    home_strength: row.home_strength, away_strength: row.away_strength,
    expected_home_goals: row.expected_home_goals, expected_away_goals: row.expected_away_goals,
    expected_home_shots: row.expected_home_shots, expected_away_shots: row.expected_away_shots,
    expected_home_sot: row.expected_home_sot, expected_away_sot: row.expected_away_sot,
    expected_corners: row.expected_corners, expected_cards: row.expected_cards,
    prob_over25: row.prob_over25, prob_btts: row.prob_btts,
    prob_corners: row.prob_corners, prob_shots: row.prob_shots,
    prob_sot: row.prob_sot, prob_cards: row.prob_cards,
    best_pick_1: row.best_pick_1, best_pick_2: row.best_pick_2,
    best_pick_3: row.best_pick_3, aggressive_pick: row.aggressive_pick,
    analysis_text: row.analysis_text,
    // [3] Form data persistido no campo data (jsonb já existente no banco)
...
async function buildAndStoreMatches(fixtureLists) {
  ...
  const allFixtures = uniqBy(
    fixtureLists.flat().filter((fixture) => {
  ...
  for (const fixture of allFixtures) {
    try {
      ...
      const baseMatchPayload = {
        id: fixture?.fixture?.id, kickoff: fixture?.fixture?.date || null,
        league: leagueDisplay, country, region: comp.region, priority: comp.priority || 70,
      ...
      if (!homeTeamId || !awayTeamId) {
        ...
        console.log(`🟡 Sem análise (time_id ausente): ${leagueDisplay} | ${baseMatchPayload.home_team} x ${baseMatchPayload.away_team}`)
        continue
      }
      ...
      if (!hasMinimumMatchData(homeContext, awayContext)) {
        stored.push(baseMatchPayload)
        console.log(`🟡 Sem análise (dados mínimos insuficientes): ${leagueDisplay} | ${baseMatchPayload.home_team} x ${baseMatchPayload.away_team}`)
        continue
      }
      ...
      if (!hasUsableMetrics) {
        stored.push(baseMatchPayload)
        console.log(`🟡 Sem análise (métricas insuficientes): ${leagueDisplay} | ${baseMatchPayload.home_team} x ${baseMatchPayload.away_team}`)
        continue
      }
      ...
      const candidates = buildCandidateMarkets({
        ...
        metrics: metricsExp, probabilities, isNationalTeamsGame,
      })

      if (!candidates.length) {
        stored.push(baseMatchPayload)
        console.log(`🟡 Sem análise (sem mercados coerentes): ${leagueDisplay} | ${baseMatchPayload.home_team} x ${baseMatchPayload.away_team}`)
        continue
      }
      ...
      const analyzedMatchPayload = {
        ...
        markets, metrics, pick: mainPick.market, probability: mainPick.probability, insight,
      }
      ...
      // [3] Monta form_data com perfis completos de casa/fora para o frontend
      const formData = {
        home_form_general: {
          matches: homeContext.general.matches,
          avgGoalsFor: homeContext.general.avgGoalsFor,
          avgGoalsAgainst: homeContext.general.avgGoalsAgainst,
          avgShots: homeContext.general.avgShots,
          avgCorners: homeContext.general.avgCorners,
          avgCards: homeContext.general.avgCards,
          recentScores: homeContext.general.recentScores,
          formStreak: homeContext.general.formStreak,
        },
        home_form_home: {
          matches: homeContext.home.matches,
          avgGoalsFor: homeContext.home.avgGoalsFor,
          avgGoalsAgainst: homeContext.home.avgGoalsAgainst,
          avgShots: homeContext.home.avgShots,
          avgCorners: homeContext.home.avgCorners,
          avgCards: homeContext.home.avgCards,
          recentScores: homeContext.home.recentScores,
          formStreak: homeContext.home.formStreak,
        },
        home_form_away: {
          matches: homeContext.away.matches,
          avgGoalsFor: homeContext.away.avgGoalsFor,
          avgGoalsAgainst: homeContext.away.avgGoalsAgainst,
          avgShots: homeContext.away.avgShots,
          avgCorners: homeContext.away.avgCorners,
          avgCards: homeContext.away.avgCards,
          recentScores: homeContext.away.recentScores,
          formStreak: homeContext.away.formStreak,
        },
        away_form_general: {
          matches: awayContext.general.matches,
          avgGoalsFor: awayContext.general.avgGoalsFor,
          avgGoalsAgainst: awayContext.general.avgGoalsAgainst,
          avgShots: awayContext.general.avgShots,
          avgCorners: awayContext.general.avgCorners,
          avgCards: awayContext.general.avgCards,
          recentScores: awayContext.general.recentScores,
          formStreak: awayContext.general.formStreak,
        },
        away_form_home: {
          matches: awayContext.home.matches,
          avgGoalsFor: awayContext.home.avgGoalsFor,
          avgGoalsAgainst: awayContext.home.avgGoalsAgainst,
          avgShots: awayContext.home.avgShots,
          avgCorners: awayContext.home.avgCorners,
          avgCards: awayContext.home.avgCards,
          recentScores: awayContext.home.recentScores,
          formStreak: awayContext.home.formStreak,
        },
        away_form_away: {
          matches: awayContext.away.matches,
          avgGoalsFor: awayContext.away.avgGoalsFor,
          avgGoalsAgainst: awayContext.away.avgGoalsAgainst,
          avgShots: awayContext.away.avgShots,
          avgCorners: awayContext.away.avgCorners,
          avgCards: awayContext.away.avgCards,
          recentScores: awayContext.away.recentScores,
          formStreak: awayContext.away.formStreak,
        },
      }

      await upsertMatchAnalysis({
        match_id: analyzedMatchPayload.id,
        home_strength: round(homeProfile.avgGoalsFor * 1.4 + homeProfile.avgShotsOnTarget * 0.55 + homeProfile.avgCorners * 0.25),
        away_strength: round(awayProfile.avgGoalsFor * 1.35 + awayProfile.avgShotsOnTarget * 0.52 + awayProfile.avgCorners * 0.23),
        ...
        best_pick_2: pick2,
        best_pick_3: pick3,
        aggressive_pick: candidates.find((x) =>
          x.market === "Mais de 2.5 gols" || x.market === "Ambas marcam" ||
          x.market.includes("Mais de 8.5 escanteios") ||
          x.market.includes("Mais de 19.5 finalizações") ||
          x.market.includes("Mais de 6.5 finalizações no gol")
        )?.market || null,
        analysis_text: insight,
        form_data: formData, // [3] persistido no campo data jsonb
      })

      stored.push({ ...analyzedMatchPayload, game_profile: gameProfile })
      console.log(`✅ ${leagueDisplay} | ${analyzedMatchPayload.home_team} x ${analyzedMatchPayload.away_team} | ${mainPick.market}`)
...
async function rebuildDailyPicks(matches) {
  ...
  const rows = selected.map((m, index) => ({
    rank: index + 1, match_id: m.id, league: m.league,
    home_team: m.home_team, away_team: m.away_team,
    market: m.pick, probability: round(m.probability),
    kickoff: m.kickoff, is_opportunity: false,
    home_logo: m.home_logo || null, away_logo: m.away_logo || null,
    created_at: new Date().toISOString(),
  }))
...
async function run() {
  console.log("🚀 Scoutly Sync V13.3 iniciado")
  console.log("📈 Melhorias: [1] Thresholds elevados [2] Team corners reativados [3] Form data persistido")
  ...
  console.log("✅ Scoutly Sync V13.3 concluído")
}

run().catch((error) => {
  console.error("❌ Erro fatal no Scoutly Sync V13.3:", error)
  process.exit(1)
})
